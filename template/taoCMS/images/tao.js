...
function backcomment(msg){
	$track('comment').value=$track('comment').value+"@"+msg;
}
function $trackajax(){
	var url=arguments[0]||""; 
		...
		if(200==xmlHttp.status){
			...
			if(resData[0].no!=-1){
				if(resData[0].msg=='顶成功')
					$track('dignum').innerHTML=parseInt($track('dignum').innerHTML)+1;
				else
					$track('dignum').innerHTML=parseInt($track('dignum').innerHTML)-1;
			}
		}
...
window.onload = function() {
	...
	if(getCookie("author")){
	$track('name').value=getCookie("author");
	$track('emails').value=getCookie("email");
	$track('websites').value=getCookie("url")?getCookie("url"):"";
	}
}