...
class Dbclass {
	...
	function connect($dbhost,$pconnect = 0){
		...
		if(RUNONSAE){
			...
			$dbconfig=explode('|',$dbhost);
			if(count($dbconfig)<4)Base::showmessage( 'TrackCMS未被正确安装或配置导致无法读取数据'.$this->error().$this->errno() , WEBURL . 'install.php' );
			$dbhost=$dbconfig[1];
			$dbuser=$dbconfig[2];
			$dbpw=$dbconfig[3];
			$dbname=$dbconfig[4];
	...
	function halt(){
		if(in_array($this->errno(),array(1049,1146,2002,1046))){
			Base::showmessage( 'TrackCMS未被正确安装或配置导致无法读取数据'.$this->error().$this->errno() , WEBURL . 'install.php' );
		}
